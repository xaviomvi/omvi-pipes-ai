...
interface ConnectorUrlFormProps {
...
const ConnectorUrlConfigForm = forwardRef<ConnectorUrlConfigFormRef, ConnectorUrlFormProps>(
  ({ onValidationChange, onSaveSuccess }, ref) => {
    ...
    const validateField = (name: string, value: string) => {
      ...
      if (name === 'url') {
        if (!value.trim()) {
          error = 'Connector URL is required';
    ...
    const handleSave = async () => {
      ...
      try {
        ...
        const errorMessage = 'Failed to save Connector Public DNS configuration';
        ...
        >
          ...
          <Box>
            <Typography variant="body2" color="text.secondary">
              Enter the Connector Public DNS that your application will connect to. Include the
        ...
        <Grid container spacing={2.5}>
          <Grid item xs={12}>
            <TextField
              ...
              helperText={errors.url || 'The URL of your connector server'}
              required
              size="small"
              disabled={!isEditing}
              InputProps={{
...
export default ConnectorUrlConfigForm;
