...
const AtlassianConfig = () => {
  ...
  const connectorNames = ['CONFLUENCE', 'JIRA'];
  ...
  const checkConnectorConfigurations = useCallback(async () => {
    ...
    try {
      ...
      const results = await Promise.allSettled([fetchConnectorConfig('atlassian')]);

      // Check if each configuration has required fields
      // Ensure property names match what's returned by the API
      const atlassianConfigured = results[0].status === 'fulfilled' && results[0].value.isConfigured;
  ...
  useEffect(() => {
    ...
    CONNECTORS_LIST.filter((connector) => connector.id === 'atlassian').forEach((connector) => {
    ...
    <Container maxWidth="lg" sx={{ py: 3 }}>
      ...
      >
        ...
        <Grid container spacing={2}>
          {CONNECTORS_LIST.filter((connector) => connector.id === 'atlassian').map((connector) => {
      ...
      <ConfigureConnectorDialog
        ...
        isEnabled={connectorStatus[currentConnector || 'atlassian']}
      />
