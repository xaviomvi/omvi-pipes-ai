...
class SyncTasks:
    ...
    def __init__(self, celery_app: CeleryApp, drive_sync_service, gmail_sync_service):
    ...
    def setup_tasks(self) -> None:
        ...
        def schedule_next_changes_watch_task(task_self, user_email: str) -> bool:
            try:
                ...
                logger.error(f"❌ Failed to schedule changes watch: {exc}")
                task_self.retry(
    ...
    async def drive_scheduled_sync_control(self, action: str) -> bool:
        ...
        try:
            ...
            if not is_within_time_range:
                logger.info("Current time %s is outside of sync window (%s-%s)",
            ...
            logger.error(f"Error in scheduled sync control: {str(e)}")
            return False
    ...
    async def drive_manual_sync_control(self, action: str, org_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error in manual sync control: {str(e)}")
    ...
    async def drive_schedule_next_changes_watch(self, user_email: str) -> bool:
        ...
        try:
            ...
            if not user_email:
                logger.error("❌ Invalid user email provided")
                return False
            ...
            if not await self.drive_sync_service.drive_service.impersonate_user(user_email):
                logger.error(f"❌ Failed to impersonate user {user_email}")
                return False
